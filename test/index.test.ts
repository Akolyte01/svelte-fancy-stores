...
describe('test mode', () => {
  ...
  it('is very interesting', async () => {
    const asyncReadableParent: Loadable<string> = asyncReadable(
      undefined,
      () => {
        console.log('asyncReadableParent');
        mockLoad();
    ...
    const asyncWritableChild = asyncWritable(
      asyncReadableParent,
      (storeValue) => {
        console.log('asyncWritableChild');
        asyncWritableChildLoad();
        return `derived from ${storeValue}`;
      },
      () => Promise.resolve(),
      false
    );
    ...
    const asyncWritableGrandChild = asyncWritable(
      asyncWritableChild,
      (storeValue) => {
        console.log('asyncWritableGrandChild');
    ...
    const derivedStore = derived(
      asyncWritableGrandChild,
      ($asyncWritableGrandChild) => $asyncWritableGrandChild
    );
